export module cairn.utils.arguments;
...
#ifdef _WIN32 
export using ArgumentStringView = std::wstring_view;
export using ArgumentString = std::wstring;
#else
export using ArgumentStringView = std::u8string_view;
export using ArgumentString = std::u8string;
#endif
...
export inline ArgumentString string_arg(std::string_view str) {
    ArgumentString ret;
    for (char c: str) ret.push_back(c);
    return ret;
}

export inline ArgumentString path_arg(const std::filesystem::path &p) {
#ifdef _WIN32 
...
export template<typename AppendableContainer>
